...
| 6 | **Judge** | Score only the dimensions declared up front (QSOS): competency test, innovation-token cost, health (`python3 "$SKILL_DIR/scripts/provenance.py" --input <owners.json> --now <UTC-timestamp>` for maintainer signal, given fetched raw GitHub data, + Scorecard from the sweep output), license bucket (flag AGPL/SSPL/no-LICENSE explicitly), fence check, reversibility. Stars are the weakest signal — never rank on them. Record confidence explicitly: name every evidence gap, degraded lane (any sweep `WARN`, or a doctor `WARN` whose detail reports an actual problem — doctor's scorecard/grep.app checks always WARN by design and are only degraded when their message says unavailable, not when it says reachable), and unresolved ambiguity — these carry into the gate and the record, never silently dropped. | `$SKILL_DIR/references/judge.md` |
