...
- `authority_receipts` entries (stable receipt ID, authenticated principal, issuer and
  auth method, project/hunt IDs, policy version, `approval_material_sha256`, exact
  component IDs and hashes, decision, nonce, issued/expiry times, issuer key ID,
  signature) are validated by the same schema family as the packet itself.
...
or "BUILD" per §3 — the typed packet and its route components are the source of truth
...
or the authority-receipt field list; or if a real hunt produces a packet shape the schema
rejects for a reason that isn't one of the five validation rules stated in or derived from
§5.4: the first three from its closing paragraph; rule 4 structurally encodes the
"source without reuse rights ... cannot be forked or vendored" sentence, added after a
review found the rights dimension never constrained the route dimension; rule 5 closes a
gap where `custom-build` could hide custom behavior under `agent-authorized` and a
`proposed` packet could pre-bake post-resolution authority outcomes.

## Receipts

- Registry audit: 31 recorded decisions, 12 distinct verdict strings, multiple
  `DEPEND`+`BUILD`-style composites.
- `docs/design.md` §5.4: dimension table, packet/record split, and the packet-validation
  rules this schema encodes.
